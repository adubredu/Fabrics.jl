 ...
 * Version 1.X  30 Apr 2020 Agility Robotics
 */
...
/* Version history:
   ...
   1.X  30 Apr 2020  Replaced pthread_once with GCC/clang constructor attribute
 */
...
static void __attribute__((constructor)) crc32c_init_sw_little(void) {
    for (unsigned n = 0; n < 256; n++) {
        uint32_t crc = n;
...
static void __attribute__((constructor)) crc32c_init_sw_big(void) {
    for (unsigned n = 0; n < 256; n++) {
        uint32_t crc = n;
        crc = crc & 1 ? (crc >> 1) ^ POLY : crc >> 1;
        crc = crc & 1 ? (crc >> 1) ^ POLY : crc >> 1;
